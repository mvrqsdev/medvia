...
const useStyles = makeStyles((theme) => ({
  ...
  const handleReopenTicket = async (id) => {
    ...
    const handleAcepptTicket = async (id) => {
        ...
        const msg = `{{ms}} *{{name}}*, sou o *${user?.name}* e vou lhe atender!`;
        const message = {
            read: 1,
            fromMe: true,
            mediaUrl: "",
      ...
      >
            ...
            <span className={classes.contactNameWrapper}>
              ...
              > {ticket.lastMessage.includes('data:image/png;base64') ? <MarkdownWrapper> Localização</MarkdownWrapper> : <MarkdownWrapper>{ticket.lastMessage}</MarkdownWrapper>}
                <span className={classes.secondaryContentSecond} >
                  {ticket?.whatsapp?.name ?  <Tooltip title={`${ticket?.whatsapp?.name?.toUpperCase()}`}>
              <WhatsApp
                fontSize="small"
                style={{
                marginRight: 5,
                ...
                color: "primary"
   			  ...
   			  />
                  ...
                  {ticketUser ? <Badge style={{ backgroundColor: "#b12bff" }} className={classes.connectionTag}>{ticketUser}</Badge> : <br></br>}
        ...
        <span className={classes.secondaryContentSecond} >
          {ticket.status === "pending" && (
            <ButtonWithSpinner
              //color="primary"
              style={{ backgroundColor: 'green', color: 'white', marginBottom: '5px', width: '77px' }}
              variant="contained"
          ...
          {(ticket.status !== "closed") && (
            <ButtonWithSpinner
              // color="primary"
              style={{ backgroundColor: 'red', color: 'white', width: '77px'}}
              variant="contained"
              className={classes.bottomButton}
              size="small"
              loading={loading}
