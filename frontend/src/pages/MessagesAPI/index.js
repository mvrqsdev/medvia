...
import { Button, Checkbox, CircularProgress, Grid, TextField, Typography} from "@material-ui/core";
...
const MessagesAPI = () => {
  ...
  const getEndpointSendGroup = () => {
    return process.env.REACT_APP_BACKEND_URL + '/api/messages/sendGroup'
  }
  const getEndpointGetContacts = () => {
    return process.env.REACT_APP_BACKEND_URL + '/api/messages/getContacts'
  }
  const getEndpointListUsers = () => {
    return process.env.REACT_APP_BACKEND_URL + '/users/list'
  }
  ...
  const handleGetContacts = async (values) => {
    var options = {
      method: 'GET',
      url: `${process.env.REACT_APP_BACKEND_URL}/api/chats`,
      headers: {
        Authorization: `Bearer ${values.token}`
      }
    };
  ...
  const handleListUsers = async (values) => {
    var options = {
      method: 'GET',
      url: `${process.env.REACT_APP_BACKEND_URL}/api/users/list`,
      headers: {
        Authorization: `Bearer ${values.token}`
      }
    };
  ...
  const handleSendTextMessageGroup = async (values) => {
    ...
    var options = {
      method: 'POST',
      url: `${process.env.REACT_APP_BACKEND_URL}/api/messages/sendGroup`,
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${values.token}`
      },
  ...
  const renderFormMessageTextGroup = () => {
    return (
      <Formik
        initialValues={formMessageTextData}
        enableReinitialize={true}
        onSubmit={(values, actions) => {
          setTimeout(async () => {
            await handleSendTextMessageGroup(values);
            actions.setSubmitting(false);
            actions.resetForm()
          }, 400);
        }}
  ...
  const renderFormGetContacts = () => {
    return (
      <Formik
        ...
        onSubmit={(values, actions) => {
          setTimeout(async () => {
            await handleGetContacts(values);
            actions.setSubmitting(false);
            actions.resetForm()
          }, 400);
        }}
  ...
  return (
    ...
    >
      ...
      <Grid container>
        <Grid item xs={12} sm={6}>
          <Typography className={classes.elementMargin} component="div">
            <p>Seguem abaixo a lista de informações necessárias para envio das mensagens de texto para contatos com Mídia:</p>
            <b>Endpoint: </b> {getEndpoint()} <br />
            <b>Método: </b> POST <br />
      ...
      <Grid container>
        <Grid item xs={12} sm={6}>
          <Typography className={classes.elementMargin} component="div">
            <p>Seguem abaixo a lista de informações necessárias para envio das mensagens de texto para grupos:</p>
            <b>Endpoint: </b> {getEndpointSendGroup()} <br />
            <b>Método: </b> POST <br />
            <b>Headers: </b> X_TOKEN (token cadastrado) e Content-Type (multipart/form-data) <br />
            <b>Body: </b> {"{ \"number\": \"558599999999\", \"body\": \"Sua mensagem\" , \"isGroup\": true }"}
          </Typography>
        </Grid>
        <Grid item xs={12} sm={6}>
          <Typography className={classes.elementMargin}>
      ...
      <Grid container>
        <Grid item xs={12} sm={6}>
          <Typography className={classes.elementMargin} component="div">
            <p>Seguem abaixo a lista de informações necessárias para obter os contatos e grupos:</p>
            <b>Endpoint: </b> {getEndpointGetContacts()} <br />
            <b>Método: </b> POST <br />
            <b>Headers: </b> X_TOKEN (token cadastrado)<br />

          </Typography>
        </Grid>
        <Grid item xs={12} sm={6}>
          <Typography className={classes.elementMargin}>
            <b>Teste do Metodo (Resposta no console)</b>
          </Typography>
          {renderFormGetContacts()}
        </Grid>
      </Grid>
      <Typography variant="h6" color="primary" className={classes.elementMargin}>
        5. Listar usuários
      ...
      <Grid container>
        <Grid item xs={12} sm={6}>
          <Typography className={classes.elementMargin} component="div">
            <p>Seguem abaixo a lista de informações necessárias para obter a lista de usuários:</p>
            <b>Endpoint: </b> {getEndpointListUsers()} <br />
            <b>Método: </b> POST <br />
            <b>Headers: </b> X_TOKEN (token cadastrado)<br />
