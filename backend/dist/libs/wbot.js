    ...
    return new Promise(async (resolve, reject) => {
        try {
            (async () => {
                ...
                logger_1.logger.info(`Inicia a sessão ${name}`);
                let retriesQrCode = 0;
                let wsocket = null;
                ...
                wsocket = (0, baileys_1.default)({
                    ...
                    browser: ["Medvia Atendimento", "Chrome", "10.15.7"],
                ...
                wsocket.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
                    // logger.info(
                    //   `Socket  ${name} Conexão ${connection || ""} ${lastDisconnect || ""
                    //   }`
                    // );
                    if (connection === "close") {
                    ...
                    if (qr !== undefined) {
                        if (retriesQrCodeMap.get(id) && retriesQrCodeMap.get(id) >= 3) {
                            ...
                            logger_1.logger.info(`QRCode de sessão Gerado ${name}`);
